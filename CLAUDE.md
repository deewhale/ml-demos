...
<!-- last-verified: 2026-05-26 (batch_v30 启动 + v29 完成 a1_beat=23.3% 小幅回落 + reached_boss=70% 再创新高; plateau bimodal 模式继续, ESCALATION 仍待 user 决策) -->
...
完整 v15-v29 细节 + audit findings 详见 [docs/v8_training_log.md](docs/v8_training_log.md)。

**[ESCALATION] N=3 plateau 触发, 归因方向待 user 决策**: v22 peak 30% 后, v23-v29
连续 7 批 bimodal 抖动 (16.7% / 30% / 3.3% / 13.3% / 16.7% / 30% / **23.3%**), ceiling 卡 30%
不突破。won_game 持续 0%, boss_kill 持续全 0 (能到 boss 杀不掉)。
autonomous loop 在 plateau 内继续小步迭代 (v30 已起), **等 user 决策归因方向**:
...
- **`batch_v30` 启动 (2026-05-26, 续训, plateau 内自动起下批)**：
  从 v29 final ckpt 续训。**autonomous loop 在 plateau 内继续小步迭代, 等 user 决策归因方向**。
  ...
  - **PID**：`86395`（nohup）；log `/tmp/v8_ppo_batch_v30.log`；output
    `sts_models/v8_ppo_batch_v30/`；exit signal file `/tmp/v8_ppo_batch_v30.exit`（如有）
  - **启动校验**：`[resume] start_episode=1920, target=2048 (将增量训 128 ep)`，0 Traceback
  - **预期**：~2-2.5h 训练 + final eval

接手 monitor 的检查清单（v30）：
- ckpt 落盘进度：`ls sts_models/v8_ppo_batch_v30/`
- 训练是否还活：`ps -ef | grep v8_ppo_train.py | grep -v grep`
- 异常监测：`grep -cE "\[guard_cap\]|MysteriousSphere event_phase=COMBAT_WON|Error|Traceback" /tmp/v8_ppo_batch_v30.log`
- 当前 ep：`grep "\[heartbeat\]" /tmp/v8_ppo_batch_v30.log | tail -1`
