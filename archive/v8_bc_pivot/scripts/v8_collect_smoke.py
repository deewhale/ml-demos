...
def main() -> int:
    ...
    if bot.phase_counts:
        print("  phase_counts          :")
        for ph, n in sorted(bot.phase_counts.items(), key=lambda x: -x[1]):
            print(f"    {ph:>20s} : {n}")
    if result.get("error"):
        print(f"  game error   : {result['error']}")

    # ---- 诊断：v8_evaluator monkey-patch 是否真的被 StSRLSolver 调用 ----
    try:
        import json
        from v8_evaluator import get_debug_stats
        print("=" * 60)
        print("V8 evaluator DEBUG STATS")
        print("=" * 60)
        print(json.dumps(get_debug_stats(), indent=2))
    except Exception as e:  # noqa: BLE001
        print(f"  [warn] get_debug_stats failed: {e}")
    return 0
